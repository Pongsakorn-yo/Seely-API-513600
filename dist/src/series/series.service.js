"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SeriesService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const series_entity_1 = require("./entities/series.entity");
const review_entity_1 = require("../reviews/entities/review.entity");
let SeriesService = class SeriesService {
    constructor(repo) {
        this.repo = repo;
    }
    async findOwnerId(seriesId) {
        const row = await this.repo.findOne({
            where: { id: seriesId },
            select: { ownerId: true, id: true },
        });
        return row === null || row === void 0 ? void 0 : row.ownerId;
    }
    async list(q) {
        var _a, _b;
        const page = Math.max(1, Number(q.page) || 1);
        const limit = Math.max(1, Number(q.limit) || 10);
        const qb = this.repo
            .createQueryBuilder("s")
            .leftJoin("s.owner", "o")
            .addSelect(["o.id", "o.username"]);
        if (q.search) {
            const likeSearch = `%${q.search}%`;
            qb.andWhere("(s.title ILIKE :search OR s.reviewDetail ILIKE :search)", {
                search: likeSearch,
            });
        }
        if (q.ratingCode) {
            qb.andWhere("s.ratingCode = :rc", { rc: q.ratingCode });
        }
        const [data, itemCount] = await qb
            .skip((page - 1) * limit)
            .take(limit)
            .getManyAndCount();
        const ids = data.map((d) => d.id);
        const stats = {};
        if (ids.length) {
            const rows = await this.repo.manager
                .createQueryBuilder(review_entity_1.Review, "review")
                .select("review.seriesId", "seriesId")
                .addSelect("AVG(review.score)", "avg")
                .addSelect("COUNT(review.id)", "cnt")
                .where("review.seriesId IN (:...ids)", { ids })
                .groupBy("review.seriesId")
                .getRawMany();
            for (const row of rows) {
                const seriesId = Number(row.seriesId);
                if (!Number.isFinite(seriesId))
                    continue;
                const average = Number((_a = row.avg) !== null && _a !== void 0 ? _a : 0);
                stats[seriesId] = {
                    averageScore: Number(average.toFixed(2)),
                    reviewCount: Number((_b = row.cnt) !== null && _b !== void 0 ? _b : 0),
                };
            }
        }
        const withStats = data.map((d) => ({
            ...d,
            stats: stats[d.id] || { averageScore: 0, reviewCount: 0 },
        }));
        return {
            data: withStats,
            meta: {
                page,
                limit,
                itemCount,
                pageCount: Math.ceil(itemCount / limit),
                hasNextPage: page * limit < itemCount,
            },
        };
    }
    async findOne(id) {
        var _a, _b;
        const s = await this.repo.findOne({
            where: { id },
            relations: ["owner"],
        });
        if (!s) {
            throw new common_1.NotFoundException("Series not found");
        }
        if (s.owner) {
            const owner = { ...s.owner };
            delete owner.password;
            s.owner = owner;
        }
        const statsRow = await this.repo.manager
            .createQueryBuilder(review_entity_1.Review, "review")
            .select("AVG(review.score)", "avg")
            .addSelect("COUNT(review.id)", "cnt")
            .where("review.seriesId = :seriesId", { seriesId: id })
            .getRawOne();
        const average = Number((_a = statsRow === null || statsRow === void 0 ? void 0 : statsRow.avg) !== null && _a !== void 0 ? _a : 0);
        return {
            ...s,
            stats: {
                averageScore: Number(average.toFixed(2)),
                reviewCount: Number((_b = statsRow === null || statsRow === void 0 ? void 0 : statsRow.cnt) !== null && _b !== void 0 ? _b : 0),
            },
        };
    }
    async create(data, ownerId) {
        const s = this.repo.create({ ...data, ownerId });
        return this.repo.save(s);
    }
    async update(id, data) {
        const s = await this.repo.findOne({ where: { id } });
        if (!s)
            throw new common_1.NotFoundException();
        Object.assign(s, data);
        return this.repo.save(s);
    }
    async remove(id) {
        const s = await this.repo.findOne({ where: { id } });
        if (!s)
            throw new common_1.NotFoundException();
        await this.repo.remove(s);
        return;
    }
};
exports.SeriesService = SeriesService;
exports.SeriesService = SeriesService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(series_entity_1.Series)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], SeriesService);
//# sourceMappingURL=series.service.js.map